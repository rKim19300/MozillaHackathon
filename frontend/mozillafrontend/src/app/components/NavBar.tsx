    ...
    <nav className="fixed top-0 left-0 w-full flex items-center justify-between flex-wrap bg-indigo-900 p-6 shadow-lg z-10">
      ...
      <div className={`w-full block flex-grow lg:flex lg:items-center lg:w-auto ${isMobileMenuOpen ? 'block' : 'hidden'}`}>
        <div className="text-sm lg:flex-grow">
          <Link href="/" className="block mt-4 lg:inline-block lg:mt-0 text-zinc-200 hover:text-white mr-4">
            Home
          </Link>
          <Link href="/docs" className="block mt-4 lg:inline-block lg:mt-0 text-zinc-200 hover:text-white mr-4">
            Docs
          </Link>
          <Link href="/examples" className="block mt-4 lg:inline-block lg:mt-0 text-zinc-200 hover:text-white">
            Examples
          </Link>
        </div>
      </div>
